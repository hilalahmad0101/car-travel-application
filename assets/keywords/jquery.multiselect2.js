(function($) {
	...
	$.fn.gs_multiselect = function(params) {
		...
		return this.each(function() {
			...
			var from_sel=$('<select id="leftPanelItems" multiple="multiple"></select>').get(0);
			var r=t.insertRow(0);
			if (options.buttons) {
			...
			fbutton=function (){
				...
				for (key in list_options) {
					...
					if (add) {
						...
						o_lc.appendChild($('<input type="hidden" name="keywordsNames[]" value="'+key+'">').get(0));
						// $("#keywordsList").val(key);

						// var a = [];
						// var divs = $('input[name=keywords').toArray().reverse();
						//   for ( var i = 0; i < divs.length; i++ ) {
						//     a.push( divs[ i ].innerHTML );
						//   }
						  // $( "#keywordsList" ).val( a.join( ", " ) );

						// disp( $('input[name=keywords]').toArray().reverse() );
