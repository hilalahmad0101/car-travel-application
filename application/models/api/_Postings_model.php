        ...
        if(!empty($insert_id))
        {
            ...
            if ($notifyUsers) 
            {
                foreach ($notifyUsers as $n) 
                {
                    ...
                    $notifyData = array("sender_uniid" => $uniid, "receiver_uniid" => $n->user_uniid, "otherID" => $otherID, "notify_location" => $other_location, "postingID" => $pid);
                    $this->db->insert("api_cartravel_notifications", $notifyData);
                }
            }
        }
        ...
        if(!empty($insert_id))
        {
            ...
            if ($notifyUsers) 
            {
                foreach ($notifyUsers as $n)
                {
                    echo " \n\n";
                    $this->Notifications_model->cityNotifications($n->user_DeviseTokenId, $body, $img, $title, $uniid, $pid);
                    $notifyData = array("sender_uniid" => $uniid, "receiver_uniid" => $n->user_uniid, "accID" => $accID, "notify_location" => $accbw_location, "postingID" => $pid);
                    $this->db->insert("api_cartravel_notifications", $notifyData);
                }
            }
        }
        ...
        if(!empty($insert_id))
        {
            ...
            if ($notifyUsers) 
            {
                foreach ($notifyUsers as $n) 
                {
                    echo " \n\n";
                    $this->Notifications_model->cityNotifications($n->user_DeviseTokenId, $body, $img, $title, $uniid, $pid);
                    $notifyData = array("sender_uniid" => $uniid, "receiver_uniid" => $n->user_uniid, "tpID" => $tourPacID, "notify_location" => $tour_post_location, "postingID" => $pid);
                    $this->db->insert("api_cartravel_notifications", $notifyData);
                }
            }
