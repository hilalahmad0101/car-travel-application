...
class CarPostings extends CI_Controller
{
	...
	{
        $allowed_mime_type_arr = array('jpg','JPG','png', 'PNG', 'jpeg', 'JPEG', 'gif', 'GIF', 'image/gif','image/jpeg','image/png','image/x-png');
    ...
    public function addDroppingCars()
	{
		...
		if($this->form_validation->run() == true)
		{
			...
			$journey_date=strtotime($journey_date);
			$journey_date=date("Y-m-d",$journey_date);
			...
			$pid = $this->CarPosting_model->saveDroppingCars($data);
			if(!empty($pid))
			{
				$json['error'] = "false";
	...
	public function allDroppingCars()
	{
		if($this->input->method(TRUE) == 'POST')
		{
			...
			$whereArr = array("location like " => "%$cityName%", "journey_date >=" => date("Y-m-d"), "dp_availability" => 1);
	...
	public function getAllPostingCars()
	{
		if($this->input->method(TRUE) == 'POST')
		{
			...
			$whereArr = array("pickupCity" => $sourceCity, "dropCity" => $destinationCity, "journey_date" => $journeyDate, "dp_availability" => 1);
	...
	public function deleteDroppingCars()
	{
		if($this->input->method(TRUE) == 'POST')
		{
			...
			{
				$json['error'] = "true";
			    $json['message'] = "Someone booked this Post, You cant delete";
			}
			elseif($data == 1)
			{
				$json['error'] = "false";
    ...
    public function todayAvailableCars()
	{
		...
		if($this->form_validation->run() == true)
		{
			...
			$pid = $this->CarPosting_model->saveTodayAvailableCars($data);
			if(!empty($pid))
			{
				$json['error'] = "false";
    ...
    public function editTodayAvailableCars()
	{
		...
		if($this->form_validation->run() == true)
		{
			...
			$status = $this->CarPosting_model->updateGroupPost(array("post_location" => $location), $whereArr);
