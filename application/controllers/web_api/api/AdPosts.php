...
class AdPosts extends CI_Controller
{
    ...
    public function addAdPost()
	{
		...
		if($this->input->method(TRUE) == 'POST')
		{
            ...
            if($cartravelID == "directLive")
            {
                unset($data['cartravel_id']);
                $data['ad_status'] = "Live";
                $data['payment_sts'] = "Captured";
                $data['payment_code'] = "direct_from_Dashboard";
                $data['cartravel_id'] = 'Admin';
            }
		...
		{
			$json['error'] = "true";
			$json['message'] = "Unknown Method".$this->input->method(TRUE);
		}
    ...
    public function getOneAdInfo()
    {
        if($this->input->method(TRUE) == 'POST')
        {
            $adID = xss_clean($this->input->post('adID'));
            $arr = array('adID' => $adID);
            $adInfo = $this->AdPost_model->adInformation($arr);
            if($adInfo)
            {
                $json['error'] = "false";
                $json['adInfo'] = $adInfo;
