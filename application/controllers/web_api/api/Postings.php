...
class Postings extends CI_Controller
{
    ...
    public function promoteVCards()
	{
		...
		if($this->form_validation->run() == true)
		{
			...
			$pid = $this->Postings_model->saveVC($data);
			if(!empty($pid))
			{
				$json['error'] = "false";
    ...
    public function postOthers()
	{
		...
		if($this->form_validation->run() == true)
		{
			...
			$pid = $this->Postings_model->saveOthers($data);
			if(!empty($pid))
			{
				$json['error'] = "false";
    ...
    public function postAccidentBreakdown()
	{
		...
		if($this->form_validation->run() == true)
		{
			...
			$pid = $this->Postings_model->saveAccBw($data);
			if(!empty($pid))
			{
				$json['error'] = "false";
	...
	$pPicture = "else picture";
			...
			$pid = $this->Postings_model->saveTourTravels($data);
			if(!empty($pid))
			{
				$json['error'] = "false";
   	...
   	public function editTourPackages()
	{
		...
		if($this->form_validation->run() == true)
		{
		    ...
		    $fileNames = array_filter($_FILES['tour_images']['name']);
		    $json['fileNames'] = count($fileNames);
		    if(!empty($fileNames)){ 
		        foreach($_FILES['tour_images']['name'] as $key=>$val){ 
		            // File upload path 
		            $fileName = basename($_FILES['tour_images']['name'][$key]); 
			...
			$sts = $this->CarPosting_model->updateGroupPost(array("post_location" => $postingLocation), $whereArr);
	...
	public function deleteTourPackages()
	{
		if($this->input->method(TRUE) == 'POST')
		{
			...
			{
				$json['error'] = "true";
			    $json['message'] = "Someone booked this Tour Package, You cant delete";
			}
			elseif($data == 1)
			{
				$json['error'] = "false";
	...
	public function allTourPackages()
	{
		...
		{
			$cityName = xss_clean($this->input->post('cityName'));
			$whereArr = array("tour_start_location like " => "%$cityName%");
	...
	public function allGroupPostings()
	{
		if($this->input->method(TRUE) == 'POST')
		{
			...
			$whereArr = array("p.post_location like" => "%$postLocation%", "p.posting_booking_sts" => 0, "accID" => 0, "jobID" => 0, "tendID"=> 0, "gpost_status" => "Approved");
