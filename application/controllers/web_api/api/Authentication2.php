...
class Authentication extends CI_Controller
{
    ...
    public function index()
    {
	   ...
	   //      $pwd = sha1(xss_clean('jktravels@'));
	   //      echo $pwd."</br>";

			// $uniid = str_shuffle('venkatapavan.janu@gmail.com'.$pwd.time());
	...
	public function chkUserMobile()
	{
		...
		else
		{
			...
			$sender = "CTRAVL";
			$phone  = $mobileNumber;
			$text  = $OTP." is the OTP that you have requested to login to CarTravels. Please Don't share OTP with anyone";
			$priority  = "ndnd";
			$stype  = "normal";
			...
			else
			{
				...
				$mail->setFrom('info@cartravels.com', 'CarTravels');
				$mail->addReplyTo('info@cartravels.com', 'CarTravels');
				...
				{   
					...
					$loginJson['message'] = "OTP Sent Successfully, Please check your E-Mail and Messages";
					$loginJson['mail_sts'] = 1;
				}
				else 
				{
					$loginJson['error'] = "true";
				    $loginJson['message'] = "Sorry! Unable to send OTP, Please try again.";
			...
			if($data)
			{
				if($data->user_status == 'Active')
				{
					...
					if($codeUpdateSts == true)
					{
						...
						$sender = "CTRAVL";
						$phone  = $mobileNumber;
						$text  = $pwdToken." is the OTP that you have requested to Forgot Password to CarTravels. Please Don't share OTP with anyone";
						$priority  = "ndnd";
						$stype  = "normal";
    ...
    public function updateChangePassword()
    {
		...
		if($this->form_validation->run() == true)
		{
			...
			{
				$cpJson['error'] = "false";
				$cpJson['message']['success'] = "Password updated successfully, Please Login.";
				$cpJson['status'] = 1;
			}
			else
			{
	...
	public function changePassword()
    {
		...
		$this->form_validation->set_rules('newPassword', 'Password', 'required|min_length[8]|max_length[25]');
